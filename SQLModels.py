...
from sqlalchemy import Column, Integer, String, Sequence,ForeignKey,Float,DateTime,Table,Boolean
...
class MyMixin(object):
    ...
    #@declared_attr
    #def object_id(cls):
    #    return Column('object_id', ForeignKey('objects.atid'))
    #@declared_attr
    #def object(cls):
    ...
    #@declared_attr
    #def object_id(cls):
    #    return Column(Integer, ForeignKey('objects.atid'),primary_key=True)
    #test = Column(Integer)
...
#     __table_args__= {'sqlite_autoincrement':True}
#     atid = Column(Integer,primary_key=True)
...
#     object_id = Column(Integer, ForeignKey('objects.atid'))
#
...
class Stain(Base):
    #__tablename__ = "stain"
    staintype = Column(String)
    #id = Column(Integer, primary_key=True)
    #__table_args__= {'sqlite_autoincrement':True}
    __mapper_args__ = {
        'polymorphic_on': staintype
    }
    #object_id = Column(Integer,ForeignKey('objects.atid'))
...
class Antibody(Base):
    ...
    #__table_args__= {'sqlite_autoincrement':True}
    antibodytype = Column(String)
    __mapper_args__ = {
        'polymorphic_on': antibodytype
    }
    #id = Column(Integer, primary_key=True)
    #object_id = Column(Integer,ForeignKey('objects.atid'))
...
class PrimaryAntibody(Antibody):
    __tablename__ = None
    __mapper_args__ = {
        'polymorphic_identity':'primaryantibody'
    }

    protein_name = Column(String(200))
    protein_asc_id = Column(String(200))

class SecondaryAntibody(Antibody):
    __tablename__ = None
    __mapper_args__ = {
        'polymorphic_identity':'secondaryantibody'
    }

    targetspecies_id = Column(Integer,ForeignKey('species.id'))
    targetspecies = relationship("Species",foreign_keys=[targetspecies_id])
...
class SectionImagePlan(Base):
    #__tablename__ = 'sectionimageplan'
    #__table_args__= {'sqlite_autoincrement':True}
    #__mapper_args__={'polymorphic_identity': 'sectionimageplan'}
    #object_id = Column(Integer, ForeignKey('objects.atid'),primary_key=True)
    #id = Column(Integer, primary_key=True)
...
class Image(Base):
    ...
    channel_setting_id = Column(Integer,ForeignKey('channelsetting.id'))
